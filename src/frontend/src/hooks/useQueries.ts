...
export function useListAppointments() {
  ...
  const query = useQuery<AppointmentRequest[]>({
    ...
    queryFn: async () => {
      console.log(
        "[useListAppointments] Starting query, actor available:",
        !!actor,
      );
      if (!actor) {
      ...
      try {
        ...
        console.log(
          "[useListAppointments] Success! Received appointments:",
          result.length,
        );
        return result;
      } catch (error) {
        console.error(
          "[useListAppointments] Error fetching appointments:",
          error,
        );
        throw error;
      }
    ...
    retry: (failureCount, error) => {
      console.log(
        "[useListAppointments] Retry attempt:",
        failureCount,
        "Error:",
        error,
      );
...
export function useUpdateAppointmentStatus() {
  ...
  return useMutation({
    mutationFn: async ({
      index,
      status,
    }: { index: bigint; status: AppointmentStatus }) => {
      if (!actor) throw new Error("Actor not available");
